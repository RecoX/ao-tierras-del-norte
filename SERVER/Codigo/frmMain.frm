...
Begin VB.Form frmMain 
   ...
   Caption         =   "Tierras del Norte AO Servidor OFICIAL"
   ClientHeight    =   4785
   ClientLeft      =   1950
   ClientTop       =   1815
   ClientWidth     =   5190
...
If Minutos = MinutosWs - 1 Then
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Tierras del Norte> En un minuto se realizara el WorldSave.", FontTypeNames.FONTTYPE_SERVER))
End If
...
txtChat.Text = txtChat.Text & vbNewLine & "Tierras del Norte AO> " & BroadMsg.Text
End Sub
...
Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Tierras del Norte AO> " & BroadMsg.Text, FontTypeNames.FONTTYPE_SERVER))
''''''''''''''''SOLO PARA EL TESTEO'''''''
''''''''''SE USA PARA COMUNICARSE CON EL SERVER'''''''''''
txtChat.Text = txtChat.Text & vbNewLine & "Tierras del Norte AO> " & BroadMsg.Text
End Sub
...
Static N As Byte
    ...
    End If
    If N = 2 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("Conteo> 4", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 3 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("Conteo> 3", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 4 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("Conteo> 2", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 5 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("Conteo> 1", FontTypeNames.FONTTYPE_RETO))
    End If
    If N >= 6 Then
...
Static N As Byte
    ...
    End If
    If N = 2 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("Conteo> 4", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 3 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("Conteo> 3", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 4 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("Conteo> 2", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 5 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("Conteo> 1", FontTypeNames.FONTTYPE_RETO))
    End If
    If N >= 6 Then
