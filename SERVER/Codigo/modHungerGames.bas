...
If Cupos <= 0 Then Err = "Los cupos no son válidos": HungerGamesCanCreate = False: Exit Function

If Gold <= 0 Then Err = "El oro ingresado no es válido": HungerGamesCanCreate = False: Exit Function
...
SendData SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Juegos del hambre> Han dado inicio los Juegos del Hambre! El máximo de cupos es [" & Cupos & "], para entrar solo debes pagar " & Gold & " monedas de oro." & vbNewLine & _
...
If .death <> 0 Then Err = "Estás en otro evento!": HungerGamesCanJoin = False: Exit Function
...
If .flags.Muerto <> 0 Then Err = "Estás muerto!": HungerGamesCanJoin = False: Exit Function
If .flags.SG.HungerIndex <> 0 Then Err = "Ya estás en los juegos del hambre!": HungerGamesCanJoin = False: Exit Function
...
If .Invent.NroItems <> 0 Then Err = "No debes tener ningún item en tu inventario!": HungerGamesCanJoin = False: Exit Function
