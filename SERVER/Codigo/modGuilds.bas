...
'***************************************************
    ...
    Or (UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 90) Then
        ...
        refError = "Para fundar un clan debes ser nivel 45, tener 90 skills en liderazgo y abonar un total de 25.000.000 monedas de oro."
        Exit Function
    End If
    ...
    For i = 1 To CANTIDADDECLANES
        If Not guilds(i) Is Nothing Then
            If guilds(i).RevisarElecciones Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Tierras del Norte AO> " & guilds(i).GetLeader & " es el nuevo líder de " & guilds(i).GuildName & ".", FontTypeNames.FONTTYPE_SERVER))
            End If
        End If
proximo:
    Next i
