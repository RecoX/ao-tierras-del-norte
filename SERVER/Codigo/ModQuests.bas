...
Dim QuestSlot As Byte
    ...
    With UserList(UserIndex).QuestStats.Quests(QuestSlot)
        ...
        Call WriteConsoleMsg(UserIndex, "Has aceptado la misión " & Chr(34) & QuestList(.QuestIndex).Nombre & Chr(34) & ".", FontTypeNames.FONTTYPE_DIOS)
        Call WriteConsoleMsg(UserIndex, "Para ver el progreso de tu misión tipea /INFOQUEST", FontTypeNames.FONTTYPE_DIOS)
        Call WriteConsoleMsg(UserIndex, "Recuerda que si deslogeas, perderas el progreso de tu misión", FontTypeNames.FONTTYPE_ORO)
    ...
    With QuestList(QuestIndex)
        ...
        If .RequiredOBJs > 0 Then
            For i = 1 To .RequiredOBJs
                If TieneObjetos(.RequiredOBJ(i).objindex, .RequiredOBJ(i).Amount, UserIndex) = False Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("No has conseguido todos los objetos que te he pedido.", Npclist(NpcIndex).Char.CharIndex, vbGreen))
                    Exit Sub
                End If
            Next i
        End If
        ...
        If .RequiredNPCs > 0 Then
            For i = 1 To .RequiredNPCs
                If .RequiredNPC(i).Amount > UserList(UserIndex).QuestStats.Quests(QuestSlot).NPCsKilled(i) Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("No has matado todas las criaturas que te he pedido.", Npclist(NpcIndex).Char.CharIndex, vbGreen))
                    Exit Sub
                End If
            Next i
        End If
        ...
        If .RewardOBJs > 0 Then
            ...
            If InvSlotsLibres < .RewardOBJs Then
                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("No tienes suficiente espacio en el inventario para recibir la recompensa. Vuelve cuando hayas hecho más espacio.", Npclist(NpcIndex).Char.CharIndex, vbGreen))
                Exit Sub
            End If
        End If
   
        'A esta altura ya cumplió los objetivos, entonces se le entregan las recompensas.
        Call WriteConsoleMsg(UserIndex, "¡Has completado la misión " & Chr(34) & QuestList(QuestIndex).Nombre & Chr(34) & "!", FontTypeNames.FONTTYPE_DIOS)
        ...
        If .RewardEXP Then
            ...
            Call WriteConsoleMsg(UserIndex, "Has ganado " & .RewardEXP & " puntos de experiencia como recompensa.", FontTypeNames.FONTTYPE_DIOS)
        End If
       
        'Se entrega el oro.
        If .RewardGLD Then
            UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD + .RewardGLD
            Call WriteConsoleMsg(UserIndex, "Has ganado " & .RewardGLD & " monedas de oro como recompensa.", FontTypeNames.FONTTYPE_ORO)
        End If
        ...
        If .RewardOBJs > 0 Then
            For i = 1 To .RewardOBJs
                If .RewardOBJ(i).Amount Then
                    ...
                    Call WriteConsoleMsg(UserIndex, "Has recibido " & QuestList(QuestIndex).RewardOBJ(i).Amount & " " & ObjData(QuestList(QuestIndex).RewardOBJ(i).objindex).name & " como recompensa.", FontTypeNames.FONTTYPE_ADMIN)
                End If
            Next i
        End If
